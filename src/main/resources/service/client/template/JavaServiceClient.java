...
#{foreach}( ${operation} in ${serviceClient.operations} )
	...
	private String ${operation.name}${operation.parameters.size}Path;
...
#{if}(${operation.asynchronousDestination.isEmpty()})
		...
		StringBuilder path = new StringBuilder(this.getEndpoint() + (StringUtils.isBlank(${operation.name}${operation.parameters.size}Path) ? "" : "/" + ${operation.name}${operation.parameters.size}Path) + "?");
