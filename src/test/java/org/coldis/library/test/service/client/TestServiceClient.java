...
java.lang.Long test
		...
		// Operation parameters.
		StringBuilder path = new StringBuilder(this.getEndpoint() + (StringUtils.isBlank(test51Path) ? "" : "/" + test51Path) + "?");
...
java.util.List<org.springframework.core.io.Resource> test
		...
		// Operation parameters.
		StringBuilder path = new StringBuilder(this.getEndpoint() + (StringUtils.isBlank(test71Path) ? "" : "/" + test71Path) + "?");
		final HttpMethod method = HttpMethod.POST;
		final MultiValueMap<String, String> headers = new LinkedMultiValueMap<>();
		Object body = null;
		final Map<String, Object> uriParameters = new HashMap<>();
